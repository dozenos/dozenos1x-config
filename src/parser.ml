...
module I = Dozenos1x_parser.MenhirInterpreter

let rec parse vy_inside_node lexbuf (checkpoint : Config_tree.t I.checkpoint) =
  match checkpoint with
  | I.InputNeeded _env ->
      let vy_inside_node, token = Dozenos1x_lexer.token vy_inside_node lexbuf in
...
let from_string s =
  ...
  parse vy_inside_node lexbuf (Dozenos1x_parser.Incremental.config lexbuf.lex_curr_p)
